...
const SCRIPT_ID = 'react-yunpian-captcha';
...
export default class YunPian extends React.PureComponent {
  static defaultProps = {
    className: 'i-yunpian',
    // onError: error => {},
  ...
  create = () => {
    ...
    ds.src = `${protocol}//www.yunpian.com/static/official/js/libs/riddler-sdk-0.2.2.js?_t=${new Date().getTime()}`;
  ...
  ready = event => {
    const that = this;
    // console.log('ready');
    const { className, onReady, ...options } = that.props;
    // const {  } = that.state;
    ...
    const captcha = new window.YpRiddler({
      ...options,
      container: that.dom.current,
    });
